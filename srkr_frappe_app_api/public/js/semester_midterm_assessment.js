frappe.ui.form.on('Semester Midterm Assessment', {
    refresh: function (frm) {
        ...
        // Apply column locks based on the current mode
        toggle_columns(frm);
    ...
    manual_entry_mode: function (frm) {
        toggle_columns(frm);
    },
...
function toggle_columns(frm) {
    // If Manual Mode is ON (1), columns should be Editable (read_only = 0)
    // If Manual Mode is OFF (0), columns should be Read Only (read_only = 1)
    let is_read_only = frm.doc.manual_entry_mode ? 0 : 1;

    frm.fields_dict['final_scores_summary'].grid.update_docfield_property('mid_1_total', 'read_only', is_read_only);
    frm.fields_dict['final_scores_summary'].grid.update_docfield_property('mid_2_total', 'read_only', is_read_only);
}
